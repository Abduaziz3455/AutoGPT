...
from forge.sdk import (Agent, AgentDB, ForgeLogger, Step, StepRequestBody, Task, TaskRequestBody, Workspace)
...
def summary(content):
    ...
    summary_chain = load_summarize_chain(llm=llm, chain_type='map_reduce', map_prompt=map_prompt_template,
                                         combine_prompt=map_prompt_template, verbose=True)
    output = summary_chain.run(input_documents=docs, objective="summarize text")
    return output
...
def scrape_website(url):
    ...
    data = {"url": 'https://uysot.uz/uz/jurnal'}

    # Convert Python object to JSON string
    data_json = json.dumps(data)

    # Send the POST request
    post_url = f"https://chrome.browserless.io/content"
...
# tools = [Tool(name="ScrapeWebsite", func=scrape_website, description="Scrape content from a website"), ]
system_message = SystemMessage(content="""
            You are a Sales Assistant, who can answer customer questions about houses;
            You operate within the following constraints:
            Do not make things up. 
            Don't suggest expensive apartments. 
            Do not give information about prices.
            All answers must be in Uzbek (Russian).
            
            Tips:
            If customer curious with flats send only one cheapest flat data.
            If you don't understand question say contact with admin https://t.me/Shahboz_softex. 
            """)
...
llm_agent = initialize_agent(tools=tools, llm=llm, agent=AgentType.OPENAI_FUNCTIONS, verbose=True,
...
class ForgeAgent(Agent):
    ...
    async def create_task(self, task_request: TaskRequestBody) -> Task:
        ...
        # existing_file_path = "chatbot_ready.csv"  # Update this with your file path

        # Read the content of the existing file
        # with open(existing_file_path, "rb") as file:
        #     # Create an instance of ArtifactUpload
        #     artifact_request = ArtifactUpload(file=UploadFile(file=file, filename=os.path.basename(existing_file_path)),
        #                                       relative_path="artifacts")  # Update the relative_path as needed
        #
        #     # Call the create_artifact method
        #     artifact = await super().create_artifact(task.task_id, file=artifact_request.file,
        #                                              relative_path=artifact_request.relative_path)
    ...
    async def execute_step(self, task_id: str, step_request: StepRequestBody) -> Step:
        ...
        if step.is_last:
            message = f'	✅ Final Step completed: {step.step_id} input: {step_input}'

        LOG.info(message)
        artifact = await self.db.create_artifact(task_id=task_id, step_id=step.step_id, file_name="chatbot_ready.csv",
                                                 relative_path='artifacts', agent_created=True, )
        LOG.info(f'Received input for task {task_id}: {step_request.input}')

        def customstep(query):
            result = llm_agent({"input": query})
